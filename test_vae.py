#!/usr/bin/env python
"""
Test variational autoencoder on scRNA-seq data.
...
def train_vae(model, train_loader, val_loader, n_epochs=30, lr=5e-4, device='cuda'):
    """Train the VAE."""
...
def encode_and_visualize(model, adata, X_full, sampled_indices, device='cuda'):
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(X_tensor), batch_size):
            ...
            # For VAE, we get the mean of the latent distribution
            encoder_output = model.encode(batch)
            latent = encoder_output['mu']
    ...
    with plt.rc_context():  # Use scanpy's default style
        ...
        plt.suptitle('VAE Latent Space Analysis (100 epochs)', fontsize=16, y=0.98)
        plt.tight_layout()
        plt.savefig('vae_latent_visualization.png', dpi=150, bbox_inches='tight')
        print("\nVisualization saved to vae_latent_visualization.png")
...
def main():
    ...
    model = VAE(
        ...
        activation='silu',
        dropout=0.0,  # No dropout for better comparison with AE
        bias=True,
        beta=beta,
    ...
    model, train_losses, val_losses, train_time = train_vae(
    ...
    with plt.rc_context({'figure.figsize': (12, 5)}):  # Use scanpy-like style
        ...
        plt.suptitle('VAE Training Progress', fontsize=16, y=1.02)
        plt.tight_layout()
    ...
    print("  - vae_latent_visualization.png")
